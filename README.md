...
Both stores are filled by nightly jobs, so what a deep call can see depends on
how far each has reached: `籌碼日報快取` covers the whole market per session,
while the fundamentals backfill works through the board over time. A name the
backfill has not reached yet answers with `no_annual_fundamentals` and the
institutional half of the read intact — which is the point of reporting
coverage per dimension rather than refusing to answer.
